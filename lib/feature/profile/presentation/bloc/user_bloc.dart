...
class UserCubit extends Cubit<UserState> {
  ...
  void getUserProfile() async {
    try{emit(UserLoading());
    try{final result = await profileRepository.getProfile();
    emit(UserLoaded(user: result));}catch(e){
  ...
  void updateUserProfile(Map<String,dynamic> user) async {
    emit(UserLoading());
    try{final result = await profileRepository.updateProfile(user);
    if (result == 'success') {
